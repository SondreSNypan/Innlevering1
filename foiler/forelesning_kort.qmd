...
::: {.selection-demo}
<div class="selection-wheel">
  <span style="--share:91" class="france">Frankrike</span>
  <span style="--share:4" class="spain">Spania</span>
  <span style="--share:5" class="rest">andre</span>
</div>
...
<div class="vector-space" role="img" aria-label="Forenklet todimensjonal projeksjon av et høydimensjonalt vektorrom">
  ...
  <i class="concept math muted" style="--x:69%;--y:51%">funksjon</i>
  <i class="concept structure" style="--x:68%;--y:19%">hovedstad</i>
...
<div class="attention-links"><i></i><i></i><i></i></div>
...
function apiConfig(model) {
  ...
  if (key.startsWith("sk-or-")) return { url: "https://openrouter.ai/api/v1/chat/completions", model: model };
  if (key.startsWith("sk-")) return { url: "https://api.openai.com/v1/chat/completions", model: model.replace(/^openai\//, "") };
  return null;
}
...
window.callModel = async function(model, messages, outputId, usageId, resultMode = "text") {
  ...
  if (!config) {
    output.textContent = "Ukjent nøkkelformat.";
    return;
  }
  ...
  try {
    const response = await fetch(config.url, {
      ...
      body: JSON.stringify({ model: config.model, messages: messages, max_tokens: resultMode === "integer" ? 80 : 800 })
    });
...
window.runMeaningWithTools = async function(outputId, usageId) {
  const output = document.getElementById(outputId);
  ...
  const config = apiConfig("openai/gpt-4o");
  const tools = [
    {
      type: "function",
      function: {
  ...
  try {
    ...
    for (let round = 0; round < 7 && !state.final; round++) {
      ...
      const response = await fetch(config.url, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": "Bearer " + window.API_KEY },
        body: JSON.stringify({ model: config.model, messages, tools: availableTools, tool_choice: "required", parallel_tool_calls: false, max_tokens: 300 })
      });
...
<div class="demo-shell">
  ...
  <div class="control-row">
    ...
    <button onclick="callModel('openai/gpt-4o-mini',[{role:'system',content:document.getElementById('sys2').value},{role:'user',content:document.getElementById('q2').value}],'out2','tok2')">Kjør</button>
  </div>
...
## Demo 3 — Modellduell {.demo-slide}

```{=html}
<div class="demo-shell">
  <div class="control-row">
    <input id="q3" value="Løs differensialligningen y'' + 4y = 0. Forklar stegene.">
    <button onclick="callModel('openai/gpt-4o-mini',[{role:'user',content:document.getElementById('q3').value}],'out3a','tok3a');callModel('openai/gpt-4o',[{role:'user',content:document.getElementById('q3').value}],'out3b','tok3b')">Start duellen</button>
  </div>
  <div class="duel-grid">
    <div><h3>Rask og billig</h3><div id="out3a" class="llm-out duel-output"></div><div id="tok3a" class="usage-line"></div></div>
    <div><h3>Sterkere modell</h3><div id="out3b" class="llm-out duel-output"></div><div id="tok3b" class="usage-line"></div></div>
  </div>
</div>
```

::: {.answer-reveal .fragment}
Fasit: $y=C_1\cos(2x)+C_2\sin(2x)$. Sammenlign kvalitet, tid og tokens.
:::
...
:::: {.columns}
::: {.column width="48%"}
- Det eldste kjente regelverket for byggingeniører finner vi i Hammurabis lover.
- Fra det gamle Babylonia, om lag 1770 år før Kristus.
- Lovens § 229 lød slik (Harper's oversettelse fra 1904):
...
:::
::: {.column width="48%"}
- Selv om straffene har blitt mindre strenge, er ansvarsprinsippet like grunnleggende i dag.
- Ansvar som skal **forebygge** baserer seg på risikovurdering, mens ansvaret når noe går galt baserer seg på **faktiske konsekvenser**.
:::
::::
